...
Three Claude Code Routines run on Anthropic-managed cloud infrastructure (not Render). They complement the monitor — the monitor handles real-time polling (2–5 min cycles), Routines handle low-frequency ambient tasks that don't need sub-minute response time.

| Routine | Trigger | Prompt |
|---------|---------|--------|
| Nightly PR Digest | Scheduled — 9 PM CST (3 AM UTC) | `prompts/routines/nightly-pr-digest.md` |
...
**Why not replace the monitor?** Routines have an hourly minimum schedule — too slow for 2-min deploy failure detection and 5-min CI polling. Use both layers together.
